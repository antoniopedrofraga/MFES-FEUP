...
public class Feature {
  ...
  public VDMSet getReqAndExcRestrictions() {
    ...
    if (requirements.size() + exclusions.size() > 0L) {
      for (Iterator iterator_21 = requirements.iterator(); iterator_21.hasNext(); ) {
        Feature requirement = (Feature) iterator_21.next();
        restrictions =
            SetUtil.union(
      ...
      for (Iterator iterator_22 = exclusions.iterator(); iterator_22.hasNext(); ) {
        Feature exclusion = (Feature) iterator_22.next();
        restrictions =
            SetUtil.union(
                Utils.copy(restrictions),
                SetUtil.set(
